...
class DEXAdapter(VenueAdapter):
    ...
    async def get_quote(
        ...
        """
        if self.paper_mode:
            return self._simulate_quote(
                sell_token, buy_token, sell_amount, slippage_bps
            )
        ...
        try:
            ...
            async with self._session.get(
                f"{self.base_url}/swap/v1/quote", params=params
            ) as resp:
                if resp.status != 200:
    ...
    def _simulate_swap(self, quote: SwapQuote) -> SwapResult:
        ...
        if buy_token in self._paper_balances:
            self._paper_balances[buy_token] = (
                self._paper_balances.get(buy_token, 0) + buy_amount
            )
    ...
    async def place_order(self, order: Order) -> Order:
        ...
        if self.paper_mode:
            ...
            fill_price = base_price * (
                1 + slippage if order.side.value == "buy" else 1 - slippage
            )
        ...
        if result and result.status == "success":
            order.status = OrderStatus.FILLED
            order.filled_size = (
                result.buy_amount if order.side.value == "buy" else result.sell_amount
            )
