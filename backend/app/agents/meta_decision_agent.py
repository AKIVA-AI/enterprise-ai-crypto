...
class MetaDecisionAgent(BaseAgent):
    ...
    async def _process_alert(self, data: Dict):
        ...
        if severity == "critical":
            self._system_stress["critical_alerts"] = (
                self._system_stress.get("critical_alerts", 0) + 1
            )
        elif severity == "warning":
            self._system_stress["warning_alerts"] = (
                self._system_stress.get("warning_alerts", 0) + 1
            )
    ...
    async def _make_decision(self) -> MetaDecision:
        ...
        if not self._volatility_data:
            ...
            return MetaDecision(
                global_state=GlobalTradingState.HALTED,
                strategy_states={
                    s: StrategyState.DISABLE for s in self._registered_strategies
                },
        ...
        avg_volatility = (
            sum(self._volatility_data.values()) / len(self._volatility_data)
            if self._volatility_data
            else 0
        )
        ...
        high_correlation = sum(
            1
            for c in self._correlation_matrix.values()
            if c > self._thresholds["correlation_high"]
        )
    ...
    def can_strategy_trade(self, strategy_id: str) -> bool:
        ...
        if self._current_decision.global_state == GlobalTradingState.HALTED:
            return False
        state = self._current_decision.strategy_states.get(
            strategy_id, StrategyState.DISABLE
        )
    ...
    async def on_start(self):
        ...
        logger.warning(
            f"[{self.agent_id}] Starting in HALTED state until data collected"
        )
