...
class StrategyLifecycleManager:
    ...
    def register_strategy(
        self,
        strategy_id: str,
        initial_state: StrategyLifecycleState = StrategyLifecycleState.PAPER_ONLY,
    ):
        ...
        logger.info(
            f"Registered strategy {strategy_id} with state {initial_state.value}"
        )
    ...
    ) -> Optional[StrategyStateTransition]:
        """
    ...
    def _check_active_to_quarantine(
        self, strategy: StrategyLifecycle
    ) -> Optional[StrategyStateTransition]:
        ...
        if (
            strategy.performance_vs_expectation
            < self._thresholds["performance_ratio_min"]
        ):
            reasons.append(
                f"underperformance:{strategy.performance_vs_expectation:.1%}"
            )
    ...
    def _check_quarantine_to_active(
        self, strategy: StrategyLifecycle
    ) -> Optional[StrategyStateTransition]:
        ...
        if (
            strategy.performance_vs_expectation
            >= self._thresholds["recovery_performance_min"]
        ):
    ...
    def _check_quarantine_to_disabled(
        self, strategy: StrategyLifecycle
    ) -> Optional[StrategyStateTransition]:
    ...
    def execute_transition(self, strategy_id: str, transition: StrategyStateTransition):
        ...
        if transition.to_state == StrategyLifecycleState.QUARANTINED:
            ...
            strategy.quarantine_expires_at = (
                datetime.utcnow()
                + timedelta(hours=self._thresholds["quarantine_min_hours"])
            ).isoformat()
