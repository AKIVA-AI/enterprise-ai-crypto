...
async def init_db():
    ...
    try:
        ...
        client.table("global_settings").select("id").limit(1).execute()
...
async def create_alert(
    ...
    try:
        ...
        logger.info("alert_created", title=title, severity=severity, source=source)
    except Exception as e:
...
async def activate_kill_switch(reason: str, user_id: str = "system") -> bool:
    ...
    try:
        ...
        if not settings_id:
            logger.error(
                "kill_switch_activation_failed", error="No global_settings row found"
            )
            return False
...
async def deactivate_kill_switch(user_id: str = "system") -> bool:
    ...
    try:
        ...
        if not settings_id:
            logger.error(
                "kill_switch_deactivation_failed", error="No global_settings row found"
            )
            return False
...
async def get_kill_switch_status() -> dict:
    ...
    try:
        ...
        result = (
            supabase.table("global_settings")
            .select("global_kill_switch, updated_at, updated_by")
            .limit(1)
            .execute()
        )
