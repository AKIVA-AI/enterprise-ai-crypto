...
class ConnectionManager:
    ...
    async def connect(
        self, websocket: WebSocket, stream: str, user_id: Optional[str] = None
    ):
    ...
    async def broadcast(self, stream: str, data: Dict[str, Any]):
        ...
        message = json.dumps(
            {"stream": stream, "timestamp": datetime.utcnow().isoformat(), "data": data}
        )
...
async def websocket_stream(
    websocket: WebSocket, stream_type: str, user_id: Optional[str] = Query(None)
):
    """
...
async def broadcast_market_update(symbol: str, price: float, change: float):
    ...
    await manager.broadcast(
        ...
        {"type": "price", "symbol": symbol, "price": price, "change_24h": change},
    )
...
async def broadcast_signal(signal: Dict[str, Any]):
    ...
    await manager.broadcast(
        StreamType.SIGNALS.value, {"type": "signal", "signal": signal}
    )
...
async def broadcast_arbitrage(opportunity: Dict[str, Any]):
    ...
    await manager.broadcast(
        StreamType.ARBITRAGE.value, {"type": "opportunity", "opportunity": opportunity}
    )
...
async def broadcast_portfolio_update(user_id: str, portfolio: Dict[str, Any]):
    ...
    await manager.send_to_user(
        user_id, {"type": "portfolio_update", "portfolio": portfolio}
    )
...
def get_websocket_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "total": sum(
            manager.get_connection_count(stream.value) for stream in StreamType
        ),
    }
