...
class EnhancedConfigManager:
    ...
    def load_configuration(
        self, config_name: str = "default", environment: str = "development"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self._notify_config_listeners(
                "loaded", config_name, environment, validated_config
            )

            logger.info(
                f"Configuration '{config_name}' loaded successfully for environment '{environment}'"
            )
    ...
    def _load_environment_config(
        self, config_name: str, environment: str
    ) -> Dict[str, Any]:
        ...
        if not env_config_file.exists():
            logger.info(
                f"No environment-specific config found for '{environment}', using defaults"
            )
            return {}
    ...
    def _validate_exchange_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in required_fields:
            if field not in exchange_config:
                raise ConfigurationError(
                    f"Exchange configuration missing required field: {field}"
                )
        ...
        if exchange_config["name"] not in supported_exchanges:
            raise ConfigurationError(
                f"Unsupported exchange: {exchange_config['name']}. Supported: {', '.join(supported_exchanges)}"
            )
        ...
        if exchange_config.get("pair_whitelist"):
            ...
            for pair in pairs:
                if not isinstance(pair, str) or "/" not in pair:
                    raise ConfigurationError(
                        f"Invalid pair format: {pair}. Expected format: BASE/QUOTE"
                    )
        ...
        if config.get("minimal_roi"):
            ...
            for time_key, profit_ratio in roi.items():
                try:
                    ...
                    raise ConfigurationError(
                        f"minimal_roi keys must be non-negative integers (minutes): {time_key}"
                    )

                if not isinstance(profit_ratio, (int, float)):
                    raise ConfigurationError(
                        f"minimal_roi values must be numbers: {profit_ratio}"
                    )
    ...
    def _create_default_config(self, config_name: str) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            # Risk management
            "stoploss": -0.10,
            "minimal_roi": {"0": 0.10, "60": 0.05, "120": 0.01, "240": 0.00},
    ...
    def save_configuration(
        ...
        try:
            ...
            logger.info(
                f"Configuration '{config_name}' saved for environment '{environment}'"
            )
    ...
    def update_configuration(
        ...
        try:
            ...
            # Notify listeners
            self._notify_config_listeners(
                "updated", config_name, environment, validated_config
            )

            logger.info(
                f"Configuration '{config_name}' updated for environment '{environment}'"
            )
            return validated_config
    ...
    def _get_or_create_encryption_key(self) -> bytes:
        ...
        logger.warning(
            "Generated new encryption key. In production, set CONFIG_ENCRYPTION_KEY environment variable."
        )
        return key
        ...
        def process_dict(
            ...
            for key, value in data.items():
                ...
                if isinstance(value, dict):
                    ...
                    path.endswith(field) for field in sensitive_fields
                ):
    ...
    def get_configuration_summary(
        self, config_name: str, environment: str = "development"
    ) -> Dict[str, Any]:
