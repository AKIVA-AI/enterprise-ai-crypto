...
class PortfolioEngine:
    ...
    def calculate_position_size(
        ...
        constraints = self.BOOK_CONSTRAINTS.get(
            book_type, self.BOOK_CONSTRAINTS[BookType.PROP]
        )
        ...
        if constraints.get("target_vol") and volatility:
            vol_scalar = (
                constraints["target_vol"] / volatility if volatility > 0 else 1.0
            )
    ...
    def _get_tier_exposure(self, positions: List[Position], tier: int) -> float:
        ...
        return sum(p.size * p.mark_price for p in positions if p.is_open)

    async def reallocate_capital(
        self, book_id: UUID, new_capital: float, user_id: str
    ) -> Book:
        """
