...
class MemeVentureService:
    ...
    async def get_projects(
        self, stage: Optional[MemeProjectStage] = None, limit: int = 50
    ) -> List[Dict]:
    ...
    async def update_scores(
        ...
        logger.info("meme_scores_updated", project_id=str(project_id), updates=updates)
    ...
    async def advance_stage(
        ...
        if not self._validate_stage_transition(old_stage, new_stage.value):
            raise ValueError(
                f"Invalid transition from {old_stage} to {new_stage.value}"
            )
    ...
    async def approve_go_no_go(
        self, project_id: UUID, user_id: str, approved: bool
    ) -> Dict:
    ...
    async def get_launch_checklist(self, project_id: UUID) -> Dict:
        ...
        checks = [
            ...
            },
            {
                ...
                "passed": project.get("holder_concentration", 100)
                <= self.MAX_HOLDER_CONCENTRATION,
            },
            {
        ...
        return {"project_id": str(project_id), "ready": all_passed, "checks": checks}
    ...
    def _validate_stage_transition(self, current: str, target: str) -> bool:
        ...
        valid_transitions = {
            ...
            MemeProjectStage.DUE_DILIGENCE.value: [
                MemeProjectStage.APPROVED.value,
                MemeProjectStage.CLOSED.value,
            ],
            MemeProjectStage.APPROVED.value: [
                MemeProjectStage.LIVE.value,
                MemeProjectStage.CLOSED.value,
            ],
