...
class BasisEdgeModel:
    ...
    def compute_expected_return(
        ...
        slippage_bps = (
            ...
            else self.config.default_slippage_bps
        )
        latency_buffer_bps = (
            latency_buffer_bps
            if latency_buffer_bps is not None
            else self.config.latency_buffer_bps
        )
        unwind_risk_buffer_bps = (
            unwind_risk_buffer_bps
            if unwind_risk_buffer_bps is not None
            else self.config.unwind_risk_buffer_bps
        )

        expected_basis_convergence_bps = (
            basis_bps_mid * self.config.basis_convergence_weight
        )
