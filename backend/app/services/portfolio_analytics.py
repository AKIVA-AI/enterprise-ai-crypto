...
class PortfolioAnalytics:
    ...
    async def get_portfolio_performance(
        self, book_id: Optional[UUID] = None, days: int = 30
    ) -> PerformanceMetrics:
        ...
        try:
            ...
            fills_query = (
                supabase.table("fills").select("*").gte("executed_at", cutoff_date)
            )
    ...
    async def get_exposure_breakdown(
        ...
        try:
            ...
            query = (
                ...
                .select("*, books(name, type, risk_tier)")
                .eq("is_open", True)
            )
            ...
            for pos in positions:
                ...
                exposure.by_book[book_name] = (
                    exposure.by_book.get(book_name, 0) + notional
                )

                # By asset
                instrument = pos.get("instrument", "Unknown")
                exposure.by_asset[instrument] = (
                    exposure.by_asset.get(instrument, 0) + notional
                )

                # By venue
                venue_id = pos.get("venue_id", "Unknown")
                exposure.by_venue[str(venue_id)] = (
                    exposure.by_venue.get(str(venue_id), 0) + notional
                )

                # By direction
                direction = "long" if is_long else "short"
                exposure.by_direction[direction] = (
                    exposure.by_direction.get(direction, 0) + notional
                )

                # By risk tier
                risk_tier = book.get("risk_tier", 1)
                exposure.by_risk_tier[risk_tier] = (
                    exposure.by_risk_tier.get(risk_tier, 0) + notional
                )
    ...
    async def get_risk_attribution(
        ...
        try:
            ...
            query = (
                supabase.table("positions")
                .select("*, books(name), strategies(name)")
                .eq("is_open", True)
            )
            ...
            for pos in positions:
                ...
                attr = RiskAttribution(
                    ...
                    notional=notional,
                    pnl=pnl,
                    pnl_contribution_pct=abs(pnl / total_pnl * 100)
                    if total_pnl != 0
                    else 0,
    ...
    async def _get_pnl_history(self, book_id: Optional[UUID], days: int) -> List[float]:
